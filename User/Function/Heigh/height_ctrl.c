...
{	
	...
	switch( height_ctrl_start_f )
	{
		...
		case 1:		//
				...
				if((flag.FlightMode==ULTRASONIC_High && lock==1 &&lock_spd_crl==0)|| (flag.FlightMode==ATMOSPHERE_High && lock_BARO==1 &&lock_spd_crl==0))//超声波 || 气压定高
				{
...
{
    if (flag.FlightMode==ULTRASONIC_High && lock==0)		//超声波定高锁定
		{
		...
		else if(flag.FlightMode==MANUAL_High || flag.FlightMode==ATMOSPHERE_High)	//MANUAL:手动控制；ATMOSPHERE：气压定高
		{lock=0;}

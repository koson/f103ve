...
//	float height_temp;
	...
	for(u8 i=0;i<20;i++) 
  {
	...
	if(Pressure>150000)Pressure=100000;//防止初始化时的值很大
	baro_height=(Pressure_groud-Pressure)*85;//经验计算，近似高度，单位mm
	if(baro_height < -200) baro_height= - 100;
	
	Baro_Height_Source=(Pressure_groud-qqp)*85;//经验计算，近似高度，单位mm
	if(Baro_Height_Source < -100) Baro_Height_Source= - 100;
	
	if(inum<300){inum++;Pressure_groud=Pressure;};//自动获得地面气压
